...
type PlayerSpellContainerProps = {
  title?: string;
  playerSpells: RitualData[];
  availableSpells: RitualData[];
...
export default function PlayerSpellContainer(props: PlayerSpellContainerProps) {
  ...
  const socket_spellChange = useRef<(sp: RitualData) => void>(() => {});
  ...
  function onSpellCreateSubmit(spell: RitualData) {
    ...
    api
      ...
      .then((res) => {
        const newSpell = res.data.spell as RitualData;
        setPlayerSpells([...playerSpells, { ...newSpell, sanity: spell.sanity, symbol: spell.symbol }]);
  ...
  function onAddSpell(id: number) {
    ...
    api
      ...
      .then((res) => {
        const spell = res.data.spell as RitualData;
    ...
    <Col xs={12} className='mb-3 w-100 text-center'>
      <Row>
        <Col className='data-container mx-3'>
          ...
          <Row className='mb-2'>
            <Col>Conjuração: {spell.castingTime}</Col>
          </Row>
          <Row className='mb-2'>
